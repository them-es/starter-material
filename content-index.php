...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'mdc-card mdc-shadow--3dp mdc-layout-grid__cell mdc-layout-grid__cell--align-top ' . esc_attr( $post_class ) ); ?>>
			...
			<p class="entry-meta">
				<?php
					...
					if ( comments_open() && $num_comments >= 1 ) :
						echo ' <a href="' . esc_url( get_comments_link() ) . '" class="pull-right mdc-badge" title="' . esc_attr( sprintf( _n( '%s Comment', '%s Comments', $num_comments, 'my-theme' ), $num_comments ) ) . '" data-badge="' . esc_attr( $num_comments ) . '">' . esc_html( $num_comments ) . ' <i class="material-icons">chat_bubble_outline</i></a>';
					endif;
				?>
			</p>
		<?php
	...
	<footer class="mdc-card__actions mdc-card--border">
		<a href="<?php the_permalink(); ?>" class="mdc-button mdc-button--stroked"><?php esc_html_e( 'more', 'my-theme' ); ?></a>
	</footer>
